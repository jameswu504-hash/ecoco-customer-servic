...
後台各區塊若遇到 Render 冷啟動、資料庫慢查詢或暫時性網路問題，會顯示「載入失敗」與「重試」按鈕，不會停留在「載入中」。通常等待 30 秒後按該區塊的「重試」即可恢復，不需要重新整理整頁。
...
- `/healthz` 是否正常。
- PostgreSQL 連線是否正常。
- Render logs 是否有 timeout、database error 或 API key 錯誤。
...
| 狀態 | 判斷 | 後續動作 |
| --- | --- | --- |
| 待處理 | AI 答不完整，內部還沒確認答案 | 問主管或負責窗口 |
| 已補知識 | 已確認並補進知識庫 | 下次測試同類問題 |
