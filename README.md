...
| `routes/` | 各 API 路由：客服對話、後台、知識庫、報表、LINE；內部 wiki 僅在 `APP_MODE=internal` 時啟用 |
| `services/` | RAG、prompt、報表、隱私遮罩；內部知識服務為 future module |
...
| `STAFF_KEY` | 未來內部 wiki 模式需要 | `APP_MODE=internal` 時的內部人員入口驗證 |
...
內部 Wiki / 員工訓練知識系統目前只保留規劃與受 `APP_MODE=internal` 保護的 API 雛形，production 預設不啟用。正式客服環境請維持：

```text
APP_MODE=customer
```

若未來要啟用內部知識系統，建議另開 Render service，使用同一個 codebase 但設定 `APP_MODE=internal` 與獨立 `STAFF_KEY`。相關規劃集中於 [`docs/future/internal-wiki/`](docs/future/internal-wiki/)。
